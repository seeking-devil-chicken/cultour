...
**Goal:** Install all dependencies, prepare the new `events` table, refactor the _existing_ server to TypeScript, and set up Vitest for _both_ environments.
...
- `[ ]` **Task 1.2: Database Migration**
  - **Task:** Run the `CREATE TABLE events (...)` script (from `DATABASE_SCHEMA.md`) on the development database.
...
- `[ ]` **Task 2.1.1: Eventbrite Service**
  - **Task:**
    ...
    - Implement a function (`fetchEventsByCity(city: string)`) that calls the Eventbrite API.
    - This function must **transform** the complex JSON response from Eventbrite into the simple 9-column structure of your `events` table.
- `[ ]` **Task 2.1.2: Data Population Script**
  - **Task:**
    - Create a standalone script (`server/scripts/seedEvents.ts`).
    - This script will import the `eventbriteService`, call it for target cities, and perform `INSERT` queries to populate the `events` table.
...
- `[ ]` **Task 2.1.3: API Endpoint - `GET /api/events`**
  - **Task:**
    ...
    - The controller must read the `city` from `req.query`.
    - It must query the local `events` table: `SELECT * FROM events WHERE event_city ILIKE $1`.
...
**Goal:** Build the new "Events" pages _without_ waiting for Nam, using local mock data that perfectly matches the API specification.

- `[ ]` **Task 2.2.1: Create Mock Data**
  - **Task:**
    - Create a new file: `client/src/mocks/mockData.ts`.
    - In this file, export dummy data (arrays of `Event` and `Post` objects) that **exactly matches the JSON structure** defined in the `EVENTS_API.md` spec.
    - Create a mock service (`client/src/mocks/mockApiService.ts`) with functions like `getEventsByCity(city)` and `getEventById(id)` that return your mock data (as a Promise) to simulate a real API call.
- `[ ]` **Task 2.2.2: Setup Routes & Navigation**
  - **Task:**
    - In `App.tsx`, wire the "Find events" button (or create one) to `Maps('/events')`.
...
- `[ ]` **Task 2.2.3: New Page - Event List (`/events`)**
  - **Task:**
    ...
    - Add a search bar to input a city.
    - On search, call your `mockApiService.getEventsByCity(city)` function.
    - Map over the returned array and render "Event Cards".
...
- `[ ]` **Task 2.2.4: New Page - Event Detail (`/events/:id`)**
  - **Task:**
    ...
    - Render the `eventDetails` (title, venue, time, description).
...
**Goal:** Connect the frontend to the real backend API and write comprehensive Vitest tests for _all_ features, new and old. (Nam will need to add test scripts to the root `package.json`).
...
- `[ ]` **Task 3.1: API Integration**
  - **Task:**
    ...
    - Remove the mock logic and replace it with real `fetch` calls to Nam's local server (e.g., `fetch('http://localhost:4000/api/events?city=...')`).
...
- `[ ]` **Task 3.2: Frontend Testing (Vitest)**
  - **Task:**
    ...
    - **New:** Write component tests for `EventsPage.tsx` and `EventDetailPage.tsx`.
...
- `[ ]` **Task 3.3: Backend Testing (Vitest)**
  - **Task:**
    ...
    - **New:** Write unit tests for the **new** `eventsController.ts`. (Mock the database calls).
    - **New:** Write an integration test for `GET /api/events/:id` to ensure the data combination is correct.
...
- `[ ]` **Task 3.4: Manual End-to-End Test**
  - **Task:**
    ...
    4.  Test the **new** flow: Navigate to "Events", search for a city, click an event, and verify both event details and related posts load correctly.
...
- `[ ]` Add server-side validation (e.g., ensure `city` in `req.query` is not empty).
...
- `[ ]` Add user-facing error handling (e.g., "No events found in this city," "Event not found").
- `[ ]` Check responsiveness: Ensure the new event pages look good on mobile.
- `[ ]` Remove all `console.log`s.
